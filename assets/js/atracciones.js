...
fetch(`https://api.opentripmap.com/0.1/en/places/radius?radius=${RADIUS}&lon=${LON}&lat=${LAT}&kinds=${KINDS}&limit=50&format=json&apikey=${API_KEY}`)
  ...
  .then(data => {
    ...
    lugaresFiltrados.forEach(lugar => {
      ...
      tarjeta.className = "bg-white shadow-lg rounded-lg p-5 text-left";

      tarjeta.innerHTML = `
        <h3 class="text-lg font-bold text-primary mb-1">${lugar.name}</h3>
        <p class="text-sm text-gray-600">Categoría: <span class="italic">${(lugar.kinds || "").split(",")[0]}</span></p>
      `;
